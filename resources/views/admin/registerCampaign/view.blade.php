<h1 class="text-3xl font-bold mb-4">Campaign Register DataTable</h1>

<div id="candidateFormContainer" class="hidden mb-4 bg-white p-4 rounded-lg shadow-lg w-2/4 modal">
    <h2 class="text-2xl font-bold mb-4" id="formTopic">Add Campaign Register</h2>
    <form id="candidateForm" class="">
        ...
        <div class="mb-4">
            ...
            <textarea id="details" name="details" rows="3"
                class="w-full px-4 py-2 border rounded-lg focus:ring focus:outline-none" placeholder="Details"></textarea>
        </div>
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <textarea id="remark" name="remark" rows="3"
                class="w-full px-4 py-2 border rounded-lg focus:ring focus:outline-none" placeholder="Remark"></textarea>
        </div>
...
<script>
    ...
    function populateTable(data) {
        ...
        <td class="py-2 px-4 border-b"><a href='tel:${data.tel}'>${data.tel}</a></td>
        ...
        <td class="py-2 px-4 border-b">${data.details}</td>
        <td class="py-2 px-4 border-b">${addColorSpanTag(data.is_completed)}</td>
        <td class="py-2 px-4 border-b">${data.remark}</td>
        <td class="py-2 px-4 border-b">
            <button class="bg-gray-500 bg-opacity-20 text-white px-4 py-2 rounded" style=" pointer-events: none; " onclick='edit(${data.id})'>Edit</button>
        </td>
    `;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        axios.get("{{ route('campaign-register.all') }}")
            .then(response => {
    ...
    function edit(id) {
        axios.get("{{ route('campaign-register.getCampaignRegistersById') }}", {
            params: {
                id: id
            }
        }).then(response => {
